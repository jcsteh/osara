...
INT_PTR CALLBACK WelcomeDlgProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    switch (msg)
    {
        ...
        case WM_DESTROY:
            ...
            PostQuitMessage(0);
#else
            // On macOS with SWELL, we don't need PostQuitMessage
            ...
            switch (LOWORD(wParam))
            {
                ...
                case IDC_BROWSE_PATH:
                {
                    ...
                    if (result == NSModalResponseOK)
                    {
                        ...
                        // Convert NSString to C string and set in the text field
                        const char* pathCStr = [selectedPath UTF8String];
                        SetDlgItemText(hwnd, IDC_PORTABLE_PATH, pathCStr);
                        
                        // Update installer state
                        if (g_installer)
                        {
                            g_installer->GetState().installPath = pathCStr;
                        }
                    }
#else
                    ...
                    if (pidl != NULL)
                    {
                        ...
                        {
                            SetDlgItemText(hwnd, IDC_PORTABLE_PATH, path);
                            if (g_installer)
                            {
    ...
    switch (msg)
    {
        ...
        case WM_COMMAND:
            switch (LOWORD(wParam))
            {
                case IDOK:
#ifndef _WIN32
                    // On macOS, terminate the app when installation is complete
                    [NSApp terminate:nil];
#else
                    DestroyWindow(hwnd);
#endif
