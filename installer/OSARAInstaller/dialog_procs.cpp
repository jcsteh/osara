...
static INT_PTR handleCancelButton(HWND hwnd) {
    ...
    switch (msg)
    {
        ...
        case WM_COMMAND:
            switch (LOWORD(wParam))
            {
                ...
                case IDCANCEL:
                    // The user reached this screen because OSARA is not installed
                    // at the selected location, so there is nothing to uninstall
                    // and nothing to cancel.  Terminate without the usual "Are you
                    // sure?" prompt — it would be confusing in this context.
                    mac_terminate_app();
                    return TRUE;
            }
            break;
        ...
        case WM_COMMAND:
            switch (LOWORD(wParam))
            {
                ...
                case IDC_PORTABLE_PATH:
                    ...
                    {
                        HWND hEdit = GetDlgItem(hwnd, IDC_PORTABLE_PATH);
                        int len = GetWindowTextLength(hEdit);
                        std::string path(len + 1, '\0');
                        GetWindowText(hEdit, &path[0], len + 1);
                        path.resize(len);
                    ...
                    if (g_installer)
                    {
                        if (g_installer->GetState().keymapOption == KEYMAP_INSTALL)
                        {
                            int confirm = MessageBox(hwnd,
                                translate("This will replace your current REAPER keymap (reaper-kb.ini) with the OSARA keymap. Your existing keymap will be backed up first.\n\nDo you want to continue?"),
                                translate("Replace Keymap?"),
        ...
        case WM_COMMAND:
            switch (LOWORD(wParam))
            {
                ...
                case IDC_CONTINUE:
                    ...
                    {
                        g_installer->PerformUninstallation();
                        g_installer->ShowScreen(IDD_COMPLETION);
                    }
                    return TRUE;
            ...
            if (g_installer)
            {
                if (g_installer->GetState().operationMode == MODE_INSTALL)
                {
                    if (g_installer->GetState().installationSucceeded)
                    {
                        ...
                        successMessage += std::string(translate("- OSARA keymap (KeyMaps/OSARA.ReaperKeyMap)")) + "\n";
                        successMessage += std::string(translate("- Localization files")) + "\n";

                        if (g_installer->GetState().keymapOption == KEYMAP_INSTALL)
                        {
                            successMessage += std::string(translate("- Active keymap replaced (reaper-kb.ini)")) + "\n";
